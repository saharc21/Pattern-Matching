...
__device__ int pm_init_gpu(pm_t *fsm)
{
    if (fsm == NULL)
    {
        return -1;
    ...
    slist_init_gpu(fsm->zerostate->_transitions);
...
__device__ pm_state_t * pm_goto_get_gpu(pm_state_t *state, unsigned char symbol)
{
...
__device__ int pm_goto_set_gpu(pm_state_t *from_state, unsigned char symbol, pm_state_t *to_state)
{
    ...
    slist_append_gpu(from_state->_transitions, newLableToAdd);
...
__device__ int pm_addstring_gpu(pm_t *fsm, unsigned char *stringToAdd, size_t n)
{
    if (fsm == NULL || (fsm->newstate + n) > PM_CHARACTERS || stringToAdd == NULL || n <= 0)
    {
        return -1;
    }
    ...
    {
        if (pm_goto_get_gpu(statePtr, stringToAdd[charIndexInString]) != NULL)
        {
            statePtr = pm_goto_get_gpu(statePtr, stringToAdd[charIndexInString]);
            charIndexInString++;
        }
        else
        {
    ...
    if (statePtr == fsm->zerostate || (charIndexInString + 1) <= n)
    {
        for (int i = charIndexInString; i < n; i++)
        {
            ...
            slist_init_gpu(to_state->_transitions);
            slist_init_gpu(to_state->output);

            //----- if it's the last character of the string add the string to stat's output
            if (i + 1 == n)
            {
                //----- add new node to the outputs list
                if(slist_append_gpu(to_state->output, stringToAdd) == -1){
                    return -1;
                }
            }
            if (pm_goto_set_gpu(statePtr, stringToAdd[i], to_state)== -1){
                return -1;
            }
            statePtr = to_state;
        }
...
__device__ int pm_makeFSM_gpu(pm_t *fsm)
{
    if (fsm == NULL)
    {
        return -1;
    ...
    {
        fatherState = (pm_state_t *)slist_pop_first_gpu(statesQueue);
        if (fatherState != NULL && fatherState->_transitions != NULL)
        {
            statePtr = slist_head(fatherState->_transitions);
            while (statePtr != NULL)
            {
                slist_append_gpu(statesQueue, ((pm_labeled_edge_t *)slist_data(statePtr))->state);
                while (fatherState != NULL)
                {
                    if (pm_goto_get_gpu(fatherState->fail, ((pm_labeled_edge_t *)slist_data(statePtr))->label) != NULL)
                    {
                        ((pm_labeled_edge_t *)slist_data(statePtr))->state->fail = pm_goto_get_gpu(fatherState->fail,
                ...
                slist_append_list_gpu(((pm_labeled_edge_t *)slist_data(statePtr))->state->output, ((pm_labeled_edge_t *)slist_data(statePtr))->state->fail->output);
                statePtr = slist_next(statePtr);
            }
        }
    }
...
__device__ void pm_fsm_search(pm_state_t *curState, unsigned char *string, size_t stringLength, size_t threadId)
{
    if (curState == NULL || string == NULL || stringLength == 0)
    {
        ...
        // printf("fail %p\n", ((pm_labeled_edge_t *)slist_data(slist_head(curState->_transitions)))->state->fail);
    ...
    slist_init_gpu(matchsList);

    slist_node_t *outputPtr;

    for (size_t i = 0; i < stringLength; i++)
    {
        // pm_state_t *cu_state = NULL;
        // cudaMallocManaged(&cu_state, sizeof(pm_state_t));
        ...
        if (returnState != NULL)
        {
            // curState = pm_goto_get_gpu<<< 1 , 1>>>(curState, &(*returnState), string[i]);
            if (returnState->output->size != 0)
            {
                outputPtr = slist_head(returnState->output);
                while (outputPtr != NULL)
                {
                    ...
                    matchToAdd->end_pos = threadId+i;
                    matchToAdd->fstate = curState;
                    matchToAdd->start_pos = threadId;
                    matchToAdd->pattern = (char *)slist_data(outputPtr);
                    if (slist_append_gpu(matchsList, matchToAdd) == -1){
                        return;
                    }
