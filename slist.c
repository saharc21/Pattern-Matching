...
__device__ void slist_init_gpu(slist_t *list)
{ //init the first list
    if (list == NULL)
    {
        return;
...
/** Initialize a single linked list
	\param list - the list to initialize */
void slist_init_cpu(slist_t *list)
{ //init the first list
    if (list == NULL)
    {
        return;
...
__device__ void* slist_pop_first_gpu(slist_t *list) //if there is information in first data, return pointer to the data and delete the first node. else, return null.(don't forget decrease the size of list)
{
...
__device__ int slist_append_gpu(slist_t *list, void *data) //create new node with new data ("void * data") and add it to the exist list from the 'tail side'
{
...
int slist_append_cpu(slist_t *list, void *data) //create new node with new data ("void * data") and add it to the exist list from the 'tail side'
{
    if (data == NULL || list == NULL ) 
        return -1;
    slist_node_t *add = (slist_node_t *)malloc(sizeof(slist_node_t));
...
/** \brief Append elements from the second list to the first list, use the slist_append function.
	...
	\return 0 on success, or -1 on failure
*/
__device__ int slist_append_list_gpu(slist_t *source, slist_t *destanation)//connect one list to an addional list 
{
    ...
    if (slist_head(source) == NULL && slist_head(destanation) != NULL)
    {
        slist_init_gpu(source);
        slist_node_t *temp = slist_head(destanation);
        while (temp != NULL)
        {
            slist_append_gpu(source, slist_data(temp));
...
//     if (slist_head(source) == NULL && slist_head(destanation) != NULL)
//     {
//         slist_init_cpu(source);
//         slist_node_t *temp = slist_head(destanation);
//         while (temp != NULL)
//         {
//             slist_append_cpu(source, slist_data(temp));
//             temp = slist_next(temp);
//         }
//         return 0;
//     }
